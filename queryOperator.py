...
class QueryOperator:
    ...
    def op_filter(collection, filter_key, selectivity=0.01, projected_keys=None, sharding_key=None, nb_servers=1000):
        """
        Operateur 1 & 2 : Filter (With or without sharding)

        Args:
        - collection (Collection object): the target collection to be queried (e.g., c_product).
        - filter_key (str):The attribute name used in the filter predicate (e.g., "brand", "id").
